...
def make_observation(*, chat_title: str = "家人群", confidence: float = 0.95) -> Observation:
    return Observation(
...
def test_gate_refuses_unlisted_chat() -> None:
    config = AppConfig(allowed_chat_titles=("家人群",), minimum_confidence=0.93)

    decision = SafetyGate(config).evaluate(
        make_observation(chat_title="工作群"), datetime.now(UTC), RunMode.AUTO
    )
